...
- **Pre-Execution Initialization Guard (CRITICAL)**: Before executing any Sentinel command or skill (except for `/sentinel`, `/sentinel-mb`, and `/sentinel-help`), you MUST verify that the Memory Bank has been bootstrapped by checking if `.memory-bank/active-session.json` or `.specs/` folders exist. If missing, halt execution and guide the user to run `/sentinel` or `/sentinel-mb`.
